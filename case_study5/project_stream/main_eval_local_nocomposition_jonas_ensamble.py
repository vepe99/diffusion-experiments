...
def main(cfg: EvalConfig):
    ...
    workflow_local = bf.EnsembleWorkflow(
        adapter=adapter,
        summary_networks=summary_network,
        inference_networks=inference_network,
        ensemble_size=3,
        standardize=["inference_variables", "summary_variables", "inference_conditions"],
    )
    approximators = workflow_local.approximator.approximators
    approximators['0'] = keras.models.load_model(model_path)
    approximators['1'] = keras.models.clone_model(approximators['0'])
    approximators['2'] = keras.models.clone_model(approximators['0'])
