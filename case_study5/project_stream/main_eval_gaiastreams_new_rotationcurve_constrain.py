...
#         test_data_multistream[k] = np.array(test_data_multistream[k])
    ...
    def constrain_m(z):
        if 'inference_variables' in workflow_global.approximator.standardize_layers:
            z = workflow_global.approximator.standardize_layers["inference_variables"](z, forward=False)
        m_nfw = z[..., 0]
        # Constraint is satisfied (<= 0) when m_nfw is within 1 std (1e11) of the mean (1e12)
        # Adjust 1e11 to whatever standard deviation you prefer
        return ((m_nfw - 1e12) / 0.5e12)**2 - 1.0
    ...
    global_posterior = workflow_global.compositional_sample(
                        ...
                        method=cfg.method,
                        guidance_kwargs ={
                            "constraints":[constrain_r, constrain_m],  
                            "guidance_strength": 1,},
                        steps=cfg.steps,
