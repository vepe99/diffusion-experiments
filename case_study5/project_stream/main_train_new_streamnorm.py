...
# def standardize_by_stream(training_data, sim_data, stats):
#     """
#     Standardizes in-place per stream.
#     observations[mask]: (N_j, 1000, 6)
#     mean_j/std_j:       (6,)  -> broadcasts over (N_j, 1000) automatically
#     """
#     observations = training_data[sim_data].copy() # (N, 1000, 6)
#     # stream_ids   = training_data['j'].astype(int)
#     stream_ids   = training_data['j'].astype(int).squeeze()   # (N,)


#     for j in jnp.unique(stream_ids):
#         mask   = stream_ids == j
#         mean_j = stats[f'mean_stream_{j}']        # (6,)
#         std_j  = stats[f'std_stream_{j}']         # (6,)
#         # mean_j and std_j broadcast over (N_j, 1000) -> (N_j, 1000, 6)
#         observations[mask] = (observations[mask] - mean_j) / std_j

#     training_data[sim_data] = observations
#     return training_data


def apply_augmentations_in_batches(training_data, augmentations, batch_size=512):
    """
    Apply augmentations to the full dataset in batches to avoid OOM.
    Returns a new dict with augmented data concatenated across all batches.
    """
    N = len(next(iter(training_data.values())))
    batched_results = []

    for start in tqdm(range(0, N, batch_size), desc="Applying augmentations"):
        end = min(start + batch_size, N)
        batch = {k: v[start:end] for k, v in training_data.items()}

        # apply augmentations sequentially, just like the workflow does internally
    ...
    keys = batched_results[0].keys()
    augmented_data = {
        k: np.concatenate([b[k] for b in batched_results], axis=0)
        for k in keys
    }
...
def main(cfg: TrainConfig):
    ...
    # fit without augmentations (already applied)
