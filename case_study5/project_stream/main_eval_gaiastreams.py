...
def main(cfg: EvalConfig):
    ...
    c = ChainConsumer()
    c.add_chain(Chain(samples=df, name="Global", color =colors[0]))
    # fig = c.plotter.plot()
    ...
    for stream_name in cfg.target_streams.keys():
        ...
        c.add_chain(Chain(samples=df_stream, name=f"{stream_name}", color =colors[i]))
        i += 1
    c.set_override(ChainConfig(shade=True))
    c.set_plot_config(PlotConfig(legend_kwargs={'fontsize': 25}))
    fig = c.plotter.plot()
    fig.savefig(os.path.join(cfg.base_dir, cfg.results_dir, f'global_cornerplot.pdf'))
