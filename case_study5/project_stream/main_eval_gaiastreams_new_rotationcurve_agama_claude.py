...
def main(cfg: EvalConfig):
    ...
    # FIX: ensure arrays are writable before in-place reordering
    for k in [cfg.sim_data, "attention_mask", "magnitudes", "vlos_error", "vlos_mask"]:
        test_data[k] = np.array(test_data[k])

    # FIX (spatial ordering): sort attended stars to front before truncating.
    # In the data creation, NGC3201 stars with Ra > 140 are zeroed IN PLACE (not moved to end),
    # so a naive [:300] slice may drop attended stars while keeping zero-padding in early slots.
    # Training used compact_to_attended which always put attended stars first — replicate that here.
    n_streams_raw = test_data["attention_mask"].shape[0]  # 3, before stream dim is added
    for i in range(n_streams_raw):
        mask_i = test_data["attention_mask"][i, 0, :].astype(bool)  # (N_max,)
        order = np.argsort(~mask_i)  # attended (True -> ~=False -> 0) sort before unattended
        test_data[cfg.sim_data][0, i] = test_data[cfg.sim_data][0, i][order]
        test_data["attention_mask"][i, 0] = test_data["attention_mask"][i, 0, order]
        for k in ["magnitudes", "vlos_error", "vlos_mask"]:
            test_data[k][i] = test_data[k][i][order]
    print("Applied compact_to_attended-style reordering before truncation.")
...
#         test_data_multistream[k] = np.array(test_data_multistream[k])
    ...
    for aug in augmentations:
        ...
        if aug.__name__ == "sample_obs_error":
            ...
            # FIX (sigma_vlos): vlos_error stores e_VHel for spec stars AND std_vhel for
            # non-spec stars (set during data creation). Use it for ALL real stars, not just
            # spectroscopic ones — training's _mask_vlos_jit expects sigma = stream std for
            # non-spec stars, not the KDE-sampled value. Gate on attention_mask (not vlos_mask)
            # so zero-padding slots are left unchanged.
            test_data["sigma_errors"] = np.array(test_data["sigma_errors"])
            test_data["sigma_errors"][:, :, -1] = np.where(
                np.array(test_data["attention_mask"]).reshape(target_shape).astype(bool),
                vlos_error_np,   # e_VHel for spec stars, std_vhel for non-spec stars
                test_data["sigma_errors"][:, :, -1],  # keep KDE only for zero-padding
            )
