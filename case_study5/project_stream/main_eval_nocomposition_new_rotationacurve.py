...
def main(cfg: EvalConfig):
    ...
    test_data_rotation_curve = dict(np.load('./data/plots/gala_rotcurv_multistream/rotation_curves.npz'))
    test_data['vcirc_kms'] = test_data_rotation_curve['vcirc_kms'][:, :, None] #extra dimension (n_observation, len_r_kpc, 1)
    test_data['r_kpc'] = np.tile(test_data_rotation_curve['r_kpc'], reps=(test_data['vcirc_kms'].shape[0],1))[:, :, None] #[:, :, None] #extra dimension (n_observation, len_r_kpc, 1)
    ...
    test_data = {k: test_data[k] for k in cfg.parameters_global + [cfg.sim_data, "j"] + ['vcirc_kms', 'r_kpc']}
    # Augmentation
    ...
    if 'standardization_stats.npz' in os.listdir(model_path):
        ...
        print("No existing standardization stats found. need to compute them")
        exit()
    ...
    test_data['r_kpc'] = np.tile(test_data['r_kpc'], (3, 1)).reshape(-1, test_data['r_kpc'].shape[-2], test_data['r_kpc'].shape[-1])
