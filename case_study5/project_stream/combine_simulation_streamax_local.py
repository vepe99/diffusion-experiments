...
if __name__ == "__main__":
    ...
    param_names_global_and_local = list(test_sim_config['priors_global'].keys()) + list(test_sim_config['priors_local']['Pal5'].keys())
    print("Parameter names global and local: ", param_names_global_and_local)

    data_dict = load_simulations_to_dict(
        base_directory, 
        subfolders, 
        N_simulations_per_folder, 
        param_names_global_and_local,
        n_particles_subsample=n_particles_subsample,
    )
    
    save_path = os.path.join(base_directory, f"training_data_local_{N_total}.npz")
