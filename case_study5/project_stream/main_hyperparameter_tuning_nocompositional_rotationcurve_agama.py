    ...
    try:
        ...
        for i in tqdm(range(N_sample)):
            ...
            pot_params = [
                            ...
                            dict(type='Spheroid', 
                                scaleRadius=p['a_TwoPowerTriaxial_halo'][0], 
                                densityNorm=p['rho_TwoPowerTriaxial_halo'][0],
                                gamma=p['gamma_TwoPowerTriaxial_halo'][0], 
                                alpha=1, 
                                beta=3, #fixed 
                                cutoffStrength=2, #fixed
                                outerCutoffRadius=np.inf, #fixed
                                axisRatioY=1.0, #fixed
                                axisRatioZ=p['q_TwoPowerTriaxial_halo'][0]), #halo
                            dict(type='Disk', 
                                scaleRadius=p['r_Disk'][0], 
                                scaleHeight=p['z_Disk'][0], 
                                surfaceDensity=p['Sigma_Disk'][0],
                                sersicIndex=1, #fixed
            ...
            points = np.column_stack((obs_R, obs_R*0, obs_R*0)) 
            v_circ_kms = np.sqrt(-obs_R*pot.force(points)[:,0])
...
if __name__ == "__main__":
    ...
    valid_mask_vcirc = ~np.any(np.isnan(training_data['vcirc_kms'].reshape(training_data['vcirc_kms'].shape[0], -1)), axis=1)
    ...
    # Apply the mask to all arrays
    clean_data = {key: training_data[key][valid_mask] for key in list(cfg.parameters_global) + list(cfg.inference_conditions) + ["vcirc_kms"]}
    ...
    data_dir_multistream = 'streams/data_multistream_agama/'
    N_multistream = 333
    ...
    # Clean test_data_multistream from NaNs
    sim_data_ms_array = test_data_multistream[cfg.sim_data]
    valid_mask_ms_sim = ~np.any(np.isnan(sim_data_ms_array.reshape(sim_data_ms_array.shape[0], -1)), axis=1)
    valid_mask_ms_vcirc = ~np.any(np.isnan(test_data_multistream['vcirc_kms'].reshape(test_data_multistream['vcirc_kms'].shape[0], -1)), axis=1)
    valid_mask_ms = valid_mask_ms_sim & valid_mask_ms_vcirc

    n_removed_ms = (~valid_mask_ms).sum()
    print(f"Removing {n_removed_ms}/{len(valid_mask_ms)} multistream simulations containing NaN values.")

    test_data_multistream = {key: test_data_multistream[key][valid_mask_ms] for key in test_data_multistream.keys()}
    ...
    storage_path = "./data/hyperparameter_tuning/agama/rotationcurve/"
    storage_name = JournalStorage(JournalFileStorage(os.path.join(storage_path, "optuna_diffusionmodel_agama_rotationcurve.log")))
