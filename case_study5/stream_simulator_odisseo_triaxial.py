...
from odisseo.option_classes import SimulationConfig, SimulationParams, MNParams, PlummerParams, PSPParams, TriaxialNFWParams, MN_POTENTIAL, PSP_POTENTIAL, TRIAXIAL_NFW_POTENTIAL
...
def run_simulation(prog_mass, t_end, x_c, y_c, z_c, v_xc, v_yc, v_zc, m_nfw, r_s, q1, q2, key, n_stars):
    ...
    config = SimulationConfig(N_particles = n_stars, 
                            ...
                            num_timesteps = 1000, 
                            external_accelerations=(TRIAXIAL_NFW_POTENTIAL, MN_POTENTIAL, PSP_POTENTIAL), 
    ...
    params = SimulationParams(t_end = t_end * (u.Myr).to(code_units.code_time),  
                          ...
                          TriaxialNFW_params= TriaxialNFWParams(Mvir=m_nfw * u.Msun.to(code_units.code_mass),
...
def prior_global_score(x: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    score = {
        ...
        "q1": np.zeros_like(q_1),
        "q2": np.zeros_like(q_2),
    }
...
def sample_hierarchical_stream_priors(n_streams=1):
    ...
    q1 = np.random.uniform(0.95, 1.05,)  # triaxial parameter
